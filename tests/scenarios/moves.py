...
def run(probe, cli, external):
    ...
    probe.wait(
        lambda s: any(t["id"] == moved and t["pinned"] for t in s.get("model", {}).get("tabs", []))
    )
    ...
    assert destination["workspace"] == workspace, "move changed workspace"
    ...
    probe.wait(
        lambda s: (
            s.get("window") == destination_id
            and s.get("command", 0) >= command
            and s["tabs"][0]["size"]["cols"] == cols
            and s["tabs"][0]["size"]["rows"] == rows
        ),
        any_window=True,
    )
    if remote_pane is not None:
        ...
        remote_wait(
            lambda panes: any(
                p["pane_id"] == remote_pane
                and p["size"]["cols"] == cols
                and p["size"]["rows"] == rows
                for p in panes
            ),
    ...
    if remote_pane is not None:
        remote_wait(
            lambda panes: (
                [p["pane_id"] for p in panes if p["workspace"] == workspace] == [remote_pane]
            ),
            "closing source did not leave exactly the moved destination pane alive",
        )
    return {
        "source_window": source,
        "destination_window": destination_id,
        "preserved_pane_ids": pane_ids,
...
def main():
    ...
    probe = Probe(output / "gui", gui, helper, args.domain, server=server, chrome=True)
