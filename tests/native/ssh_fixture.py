...
class LocalSshMux:
    ...
    def start(self):
        ...
        settings = {
            ...
            "HostKey": self.root / "host_key", "PidFile": self.root / "sshd.pid",
            # The owned fixture is mode0700; permit a sticky /tmp ancestor on macOS.
            "AuthorizedKeysFile": self.root / "authorized_keys", "StrictModes": "no",
            "PubkeyAuthentication": "yes", "AuthenticationMethods": "publickey",
            "PasswordAuthentication": "no", "KbdInteractiveAuthentication": "no",
            "UsePAM": "no", "AllowUsers": user, "AllowAgentForwarding": "no",
            "AllowTcpForwarding": "no", "X11Forwarding": "no", "PrintMotd": "no",
            "UseDNS": "no", "LogLevel": "VERBOSE",
        }
        (self.root / "sshd_config").write_text("".join(f"{k} {v}\n" for k, v in settings.items()))
        daemon = self.launch([sshd, "-D", "-e", "-f", str(self.root / "sshd_config")], "sshd.log")
        self.ssh = ["ssh", "-F", "/dev/null", "-i", str(self.root / "client_key"),
                    "-o", "IdentitiesOnly=yes", "-o", "BatchMode=yes", "-o",
                    f"UserKnownHostsFile={self.root}/known_hosts", "-o", "GlobalKnownHostsFile=/dev/null",
                    "-o", "StrictHostKeyChecking=yes", "-p", str(port), f"{user}@127.0.0.1"]
        ...
        while True:
            ...
            try:
                ...
                if time.monotonic() > deadline:
                    raise RuntimeError("isolated sshd did not begin listening")
                time.sleep(.05)
        subprocess.run(self.ssh + ["printf VTABS_SSH_OK"], check=True, timeout=10,
                       stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        socket_path = self.root / "mux.sock"
        config = self.root / "mux.lua"
        config.write_text("return { unix_domains={{name='ssh-fixture',socket_path="
                          + json.dumps(str(socket_path))
                          + ",no_serve_automatically=true}},default_prog={'/bin/sh'},check_for_updates=false }\n")
        remote = {f"XDG_{name}_HOME": str(self.root / name.lower())
                  for name in ("CONFIG", "CACHE", "DATA", "STATE")}
        remote.update({"XDG_RUNTIME_DIR": str(self.root / "runtime"),
        ...
        while not socket_path.exists():
            if mux.poll() is not None or time.monotonic() > deadline:
                raise RuntimeError("isolated mux startup failed: " + (self.root / "mux.log").read_text())
            time.sleep(.05)
        command = ["env"] + [f"{key}={value}" for key, value in remote.items()]
        command += [str(cli), "--config-file", str(config), "cli", "--prefer-mux", "--no-auto-start"]
        result = subprocess.run(self.ssh + [shlex.join(command + ["list", "--format", "json"])],
                                check=True, timeout=10, capture_output=True, text=True)
        json.loads(result.stdout)
        self.domain.write_text(json.dumps({
            "name": "scenario-ssh", "remote_address": f"127.0.0.1:{port}", "username": user,
            "no_agent_auth": True, "multiplexing": "WezTerm", "remote_wezterm_path": str(cli),
            "override_proxy_command": shlex.join(command + ["proxy"]),
            "ssh_option": {"identityfile": str(self.root / "client_key"),
                           "identitiesonly": "yes", "userknownhostsfile": str(self.root / "known_hosts"),
...
def main():
    ...
    parser.add_argument("--wezterm", required=True, type=Path, help="CLI binary from the tested build")
    parser.add_argument("--mux-server", type=Path)
    parser.add_argument("--output", type=Path)
    parser.add_argument("scenario", nargs=argparse.REMAINDER, help="scenarios.py arguments after --; omit to probe SSH-mux only")
    args = parser.parse_args()
    root = (args.output or Path(tempfile.mkdtemp(prefix="vtabs-localhost-ssh-"))).resolve()
    fixture = LocalSshMux(root, args.wezterm.resolve(),
    ...
    try:
        ...
        if scenario:
            subprocess.run([sys.executable, str(Path(__file__).with_name("scenarios.py")),
                            "--domain", "ssh", "--ssh-config", str(config)] + scenario, check=True)
    finally:
        fixture.close()
