...
def generate_certificates(root: Path, username: str) -> None:
    ...
    run("req", "-x509", "-newkey", "rsa:2048", "-nodes", "-days", "1",
        "-subj", "/CN=WezTerm fixture CA", "-addext", "basicConstraints=critical,CA:TRUE",
    ...
    for name, (common_name, usage, alternatives) in names.items():
        ...
        run("req", "-new", "-newkey", "rsa:2048", "-nodes", "-subj", subject,
        ...
        run("x509", "-req", "-in", str(root / f"{name}.csr"),
            "-CA", str(root / "ca.pem"), "-CAkey", str(root / "ca.key"),
            "-set_serial", str(secrets.randbits(128) or 1), "-days", "1",
            "-extfile", str(extensions), "-out", str(root / f"{name}.pem"))
...
class LocalTlsMux:
    ...
    def start(self) -> Path:
        ...
        self.domain.write_text(json.dumps({
            "name": "scenario-tls", "remote_address": f"127.0.0.1:{port}",
            "pem_cert": str(self.certificates / "client.pem"),
            "pem_private_key": str(self.certificates / "client.key"),
            "pem_root_certs": [ca], "expected_cn": "localhost",
            "accept_invalid_hostnames": False, "connect_automatically": False,
        }, indent=2) + "\n", encoding="utf-8")
        ...
        self.config.write_text(
            ...
            "default_prog={" + quote(shell) + "},unix_domains={{name='tls-fixture',socket_path="
            + quote(self.root / "mux.sock")
            + ",no_serve_automatically=true}},tls_servers={{bind_address="
            + quote(f"127.0.0.1:{port}") + ",pem_cert="
            + quote(self.certificates / "server.pem") + ",pem_private_key="
            + quote(self.certificates / "server.key") + ",pem_root_certs={"
            + quote(ca) + "}}}}\n", encoding="utf-8")
        ...
        self.process = subprocess.Popen(
            [str(self.server), "--config-file", str(self.config)], env=env,
            stdin=subprocess.DEVNULL, stdout=self.log, stderr=subprocess.STDOUT,
            start_new_session=os.name != "nt")
        context = ssl.create_default_context(cafile=ca)
        context.load_cert_chain(str(self.certificates / "client.pem"),
                                str(self.certificates / "client.key"))
        deadline = time.monotonic() + 15
        while True:
            if self.process.poll() is not None or time.monotonic() > deadline:
                raise RuntimeError("isolated TLS mux startup failed: "
...
def main() -> None:
    ...
    parser.add_argument("scenario", nargs=argparse.REMAINDER,
    ...
    try:
        ...
        print(f"TLS-mux authenticated using {fixture.protocol}; fixture: {fixture.root}", flush=True)
        ...
        if scenario:
            subprocess.run([sys.executable, str(Path(__file__).with_name("scenarios.py")),
                            "--domain", "tls", "--tls-config", str(config)] + scenario,
                           check=True)
    finally:
        fixture.close()
