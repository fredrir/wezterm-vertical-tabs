...
| Work | Optimization |
| --- | --- |
| Unchanged native snapshots | Reuse tab metadata and ordering; skip topology allocations |
...
| Native rendering | Headless integration verifies behavior; benchmark timings exclude GPU and physical display latency |
| Durability | SQLite schema, journal policy, atomic writes and revision conflicts remain unchanged |

Native integration commands: [Validation](validation.md).
