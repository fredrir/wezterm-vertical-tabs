...
fn updater() -> anyhow::Result<Option<(PathBuf, PathBuf, PathBuf)>> {
    ...
    let marker = directory.join("bundle.json");
    if !marker.is_file() {
        return Ok(None);
    }
