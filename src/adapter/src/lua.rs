...
use vtabs_app::{HookResult, HookToken, WindowApp, core};
...
/// state means a failed reload cannot replace the last valid configuration.
...
/// Cache this at a configuration epoch boundary, never on activation or paint.
pub fn hooks_enabled() -> bool {
    let result = config::run_immediate_with_lua_config(|lua| {
        let Some(lua) = lua else { return Ok(false) };
        let module = get_or_create_sub_module(&lua, "vtabs")?;
...
pub fn register(lua: &Lua) -> anyhow::Result<()> {
            ...
            |lua, (window, action): (mlua::UserDataRef<GuiWin>, mlua::Value)| {
                ...
                window
                    ...
                    .notify(TermWindowNotif::Apply(Box::new(move |tw| {
                        tw.vtabs_message_for(mux_window_id, json!({"action":action}))
                    })));
                Ok(())
            },
        )?,
    ...
    module.set("inspect", lua.create_async_function(|lua, window: mlua::UserDataRef<GuiWin>| async move {
        ...
        window.window.notify(TermWindowNotif::Apply(Box::new(move |tw| {
            tw.vtabs_sync();
            let geometry = tw.vtabs_geometry();
            let value = if let Some(ui) = tw.ui_host.as_ref() {
...
pub fn hooks(
    ...
    promise::spawn::spawn(async move {
        ...
        window.notify(TermWindowNotif::Apply(Box::new(move |tw| {
            tw.vtabs_message_for(mux_window_id, json!({"hooks":results}))
        })));
    })
    .detach();
}
