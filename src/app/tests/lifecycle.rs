...
fn snapshot(revision: u64, active: Option<u64>, tabs: Vec<Tab>) -> HostSnapshot {
    HostSnapshot {
        revision,
        active_tab: active,
        tabs,
...
#[test]
fn unchanged_paint_reuses_buffer_and_schedules_no_idle_frames() {
    let mut app = app();
...
fn reserved_spawn_activates_captured_space_when_interaction_unchanged() {
    ...
    // The snapshot already reports tab 3 active; repairing the empty projection
...
#[test]
fn close_restores_visible_successor_as_explicit_id_command() {
    let mut app = app();
    app.dispatch(Intent::CreateSpace {
        name: "Other".into(),
    })
