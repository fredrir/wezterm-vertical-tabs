...
function M.path_for(window_id, rect)
  -- The geometry is in the name, so wezterm's path+mtime image cache can never serve a stale one.
  return string.format("%s/frame-%d-%s.png", dir_for(), window_id, key_of(rect))
end
...
function M.refuses(gui_window)
  ...
  if type(theirs) == "table" and #theirs > 0 then
    ...
    if file == nil or file ~= current[gui_window:window_id()] then
      return "background"
    end
  end
...
function M.render(gui_window, cfg, rect)
  ...
  local path = M.path_for(gui_window:window_id(), rect)
  local args = {
    exe,
    "frame",
    "--w",
...
---Writes back only the keys the frame owns: `apply_titlebar_band` holds `window_padding` in the
---same table, so a whole-table write would drop whichever of the two ran first.
function M.install(gui_window, path, cfg)
  ...
  local m = M.margin(cfg)
  merged.window_padding = { left = m, right = m, top = m, bottom = m }
  local colors = {}
  for key, value in pairs(merged.colors or {}) do
    colors[key] = value
  end
  -- The divider would otherwise draw a line straight across the frame.
  colors.split = M.colours(gui_window, cfg).fill
...
function M.sync(gui_window)
  ...
  local path = M.path_for(wid, rect)
