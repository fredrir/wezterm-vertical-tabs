...
use crate::frame::{self, Cell, Rgb, Style};
...
fn row_colors(item: &LItem, theme: &Theme, st: &St) -> (Rgb, Rgb) {
    if st.dragging {
...
fn accent_of(item: &LItem, theme: &Theme) -> Rgb {
...
fn marker<'a>(item: &LItem, theme: &Theme, st: &St, glyphs: &'a Glyphs) -> (Option<&'a str>, Rgb) {
