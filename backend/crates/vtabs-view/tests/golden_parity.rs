...
fn every_scene_matches_its_goldens() {
    ...
    let mut names: Vec<String> = std::fs::read_dir(&scenes)
        ...
        .collect();
    names.sort();
    assert!(names.len() >= 17, "expected the 17 sidebar scenes, found {}", names.len());
    ...
    for name in &names {
        ...
        let want_styled = std::fs::read_to_string(frames.join(format!("{name}.styled.txt"))).unwrap();
        if text != want_text {
            failures.push(format!("{name}.txt:\n--- want\n{want_text}--- got\n{text}"));
        }
        if styled != want_styled {
            failures.push(format!("{name}.styled.txt:\n--- want\n{want_styled}--- got\n{styled}"));
        }
    }
