...
use crate::geom::BUTTON_PITCH_PT;
...
pub fn strip_actions(
    ...
    let base = if rail {
        ...
        reserve + 2
    } else if cfg.position == Position::Right {
        g.card_x1
    } else {
...
fn shows_close(item: &LItem, cfg: &RenderCfg, st: &St) -> bool {
    if cfg.close_button == CloseButton::Never || st.dragging {
        return false;
    }
    if cfg.close_button == CloseButton::Always || cfg.hover == HoverMode::Press {
        return true;
    }
...
fn card_span(item: &LItem, cfg: &RenderCfg, st: &St, g: &Grid, part: Part) -> Option<CardSpan> {
    ...
    let pin_only = item.is_pinned && cfg.pinned_style != PinnedStyle::Full;
    if part == Part::Title && pin_only {
        if cfg.pinned_style == PinnedStyle::Compact || st.hovered {
...
fn pads_for(tab_height: TabHeight) -> i64 {
    match tab_height {
        TabHeight::Row => 0,
        TabHeight::Tall => 2,
        TabHeight::Card => 1,
...
fn card_rows(item: &LItem, cfg: &RenderCfg) -> (i64, bool, i64, i64) {
    let dense = item.is_pinned && cfg.pinned_style != PinnedStyle::Full;
    let armed_dense = item.armed_pinned == Some(true) && cfg.pinned_style != PinnedStyle::Full;
...
pub fn on_inner_edge(x: i64, cols: i64, position: Position) -> bool {
...
pub fn plan<'a>(view: &'a RenderInput) -> Plan<'a> {
    ...
    if !pinned.is_empty() && cfg.separator != Separator::None {
        entries.push(if cfg.separator == Separator::Rule {
            Entry::Separator
        } else {
            Entry::Space
        });
    ...
    let mut thumb = None;
    if cfg.scroll_indicator != ScrollIndicator::Never && total > list_rows && (!rail || cols >= 7) {
        ...
        thumb = Some(Thumb {
            first: 1
                + (scroll as f64 * (list_rows - len) as f64 / max_scroll.max(1) as f64 + 0.5)
                    .floor() as i64,
            len,
            lit: cfg.scroll_indicator == ScrollIndicator::Always
                || view.hover.is_some()
