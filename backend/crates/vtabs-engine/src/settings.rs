...
use crate::text;
use document::Widget;
use presentation::{PresentationField, PresentationGroup, SettingsPresentation};
...
pub enum Shown<'a> {
    Field(&'a PresentationField),
    Caveat(&'a str),
}

impl<'a> Shown<'a> {
    fn field(self) -> Option<&'a PresentationField> {
        match self {
...
#[derive(Debug, Clone, Copy)]
pub struct Body<'a> {
    pub nav: Option<&'a PresentationGroup>,
    pub nav_selected: bool,
    pub caveat: Option<&'a str>,
    pub field: Option<&'a PresentationField>,
    pub focused: bool,
}
...
pub enum Row<'a> {
    ...
    Rule,
    Space,
    Help(Option<&'a PresentationField>),
    Hints,
    Body(Body<'a>),
}
...
pub struct Plan<'a> {
    ...
    pub groups: &'a [PresentationGroup],
    pub group: Option<&'a PresentationGroup>,
...
impl<'a> Plan<'a> {
    ...
    pub fn focused(&self) -> Option<&'a PresentationField> {
        self.shown
...
/// The pane's own facts plus the engine-owned settings presentation the widget reads.
pub struct SettingsView<'a> {
    pub cols: i64,
    pub rows: i64,
    pub presentation: &'a SettingsPresentation,
    pub ui: &'a SettingsUi,
    pub theme: Theme,
...
impl SettingsView<'_> {
    ...
    /// True while the Rust-owned document is editing a row, so the next key is a document action.
    pub fn editing(&self) -> bool {
        self.presentation.fields.iter().any(|f| f.editing.is_some())
    }

    /// True while a recorder is armed: the next key is the binding, whatever it is.
    pub fn armed(&self) -> bool {
        self.presentation.fields.iter().any(|f| f.armed)
    }
}
...
pub fn plan<'a>(v: &SettingsView<'a>) -> Plan<'a> {
    ...
    for i in 1..=body {
        ...
        if let Some(e) = entry
            && e.locked.is_none()
            && (e.widget == Widget::Picker || e.widget == Widget::Stepper)
        {
            spans.push(Span {
                id: SpanId::Dec,
                x1: g.value_x2 - 11,
    ...
    for row in 0..n {
        ...
        match plan.rows[row] {
            ...
            Some(Row::Header) => {
                ...
                let version = v.presentation.version.as_deref().unwrap_or("");
