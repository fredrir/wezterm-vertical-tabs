...
use vtabs_protocol::{DoArgs, Event};
use vtabs_theme::{Palette, Theme, UserTheme};
use vtabs_view::menu::{self, MenuCfg, MenuState, Outcome, Placed};
...
fn key(m: &Open, state: &MenuState, name: &str, mods: Mods) -> vtabs_input::Resolution {
...
fn enter_picks_the_selection_and_the_arrows_skip_what_is_disabled() {
    ...
    assert_eq!(args(&picked.events).id.as_deref(), Some("close"));
